...
2026-09-23の更新では4言語のホームにリモート撮影を追加し、専用ドラレコとの比較ページ2件を削除しました。公開状態はGitと本番サイトで確認してください。変更を公開する場合はPull Request経由で`main`へ反映します。
...
- index対象は比較ページ削除後に20ページです。別にGoogle Search Console所有権確認用HTMLが1件あります。
- 20 URLと翻訳クラスターは`tool/check_site.py`にも固定されています。URLを意図的に追加・削除する場合は、HTML・仕様書・checker・sitemap・llmsを同じ変更で更新します。
...
- 日本語ホームは「駐車監視アプリ」として製品を紹介します。比較ページと比較への内部リンクは掲載しません。
